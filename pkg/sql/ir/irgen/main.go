...
func process(tbase string, pkg string, defs defList) {
	...
	for i, d := range defs {
		...
		case union:
			...
			p.structnames = append(p.structnames, d.name)
		case rec:
...
`, p.pkg)
	...
	if len(p.unionnames)+len(tmpl) > 0 {
...
`, p.pkg)
	...
	for _, name := range p.recnames {
		...
		for _, f := range d.f {
			if p.isImmediateType(f.t) || !f.isArray {
				continue
			}
...
func (p *params) isImmediateType(t string) bool {
	_, ok1 := p.unions[t]
	_, ok2 := p.recs[t]
	return !ok1 && !ok2
}

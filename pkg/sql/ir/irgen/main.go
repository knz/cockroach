...
func main() {
	...
	basename := ifile[:len(ifile)-len(ext)]
	basetarget := filepath.Join(os.Args[2], basename)

	process(basetarget, dl)
}
...
func parse(fname string) (defList, error) {
	...
	s := Scanner{
		convPos: func(pos token.Pos) string { return fileSet.PositionFor(pos, true).String() },
	}
...
func process(tbase string, defs defList) {
	...
	genproto(tbase, allnames, m, recs, unions)
	genbridge(tbase, allnames, m, recs, unions)
	genalloc(tbase, allnames, m, recs, unions)
	genwalk(tbase, allnames, m, recs, unions)
	gensql(tbase, allnames, m, recs, unions)
}

func gensql(
	tbase string, allnames []string, m map[string]def, recs, unions map[string]struct{},
) {
	...
	fmt.Fprintln(w, `// Code generated by irgen.
...
func genproto(tbase string, allnames []string, m map[string]def, recs, unions map[string]struct{}) {

...
func (p *planner) RenameTable(ctx context.Context, n *tree.RenameTable) (planNode, error) {
	...
	dbDesc, err := MustGetDatabaseDesc(ctx, p.txn, p.getVirtualTabler(), oldTn.Catalog())
	if err != nil {
		return nil, err
	}
	...
	targetDbDesc, err := MustGetDatabaseDesc(ctx, p.txn, p.getVirtualTabler(), newTn.Catalog())
	if err != nil {
		return nil, err
	}
